...
jokers = [joker.get_rect(), joker.get_rect()]
...
while True:
    ...
    screen.blit(immunity, immunityRect)
    ...
    discardCountRect = discardCount.get_rect()
    ...
    for i in jokers:
        i.x = screenW - spacer - (cardW / 2)
        i.y = int(spacer * 1.5) + cardH + jokers.index(i) * ((cardH + spacer)/ 2)
        screen.blit(joker, i)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT: sys.exit()
            ...
            if confirmAttackRect.collidepoint(event.pos) and phase == 'attack' and len(attack) > 0:
                ...
                if diamondsAbility == True:
                    for i in range(atkPow):
                        if len(hand) < 8: hand.append(tavern.popleft())
                        else: break
            ...
            elif confirmDamageRect.collidepoint(event.pos):
            ...
            elif phase == 'attack' and endTurn == False:
                cardMoved = False
